...
pub fn init(b: *std.Build, appVersion: []const u8, libVersion: []const u8) !Config {
    ...
    config.sentry = b.option(
        ...
        switch (target.result.os.tag) {
            .macos, .ios, .maccatalyst, .visionos => break :sentry true,
    ...
    config.i18n = b.option(
        ...
        .macos, .ios, .maccatalyst, .visionos => true,
    ...
    config.emit_macos_app = b.option(
        ...
        // The rootshell xcframework variants deliberately ship no macOS slice
        // (appstore is iOS/visionOS/Catalyst, standalone is Catalyst only), so
        // the macOS app has nothing current to link against and its build fails
        // on a stale or absent libghostty. Don't build it for those targets.
        switch (config.xcframework_target) {
            .rootshell_appstore, .rootshell_standalone => break :macos_app false,
            .native, .universal => {},
        }
        break :macos_app !config.emit_lib_vt and config.emit_xcframework;
    };
...
pub fn osVersionMin(tag: std.Target.Os.Tag) ?std.Target.Query.OsVersion {
    return switch (tag) {
        ...
        // iOS 17 picked arbitrarily. Zig 0.16 models Mac Catalyst as its own
        // OS tag (it was iOS + the .macabi ABI before), versioned like iOS.
        .ios, .maccatalyst => .{ .semver = .{
            .major = 17,
            .minor = 0,
            .patch = 0,
        } },

...
fn resolveDestination(
    ...
    // iOS / visionOS cannot spawn child processes; the `ssh -G` resolution is
    // desktop-only there. Compile it out so libghostty links on those targets.
    if (comptime !std.process.can_spawn) return null;
...
fn installRemoteTerminfo(
    ...
    // iOS / visionOS cannot spawn child processes; remote terminfo install is
    // desktop-only there. Compile it out so libghostty links on those targets.
    if (comptime !std.process.can_spawn) return error.InstallFailed;
...
fn childExec(alloc: Allocator, argv: []const []const u8) !u8 {
    // iOS / visionOS cannot spawn child processes; this desktop-only exec path
    // is compiled out so libghostty links on those targets.
    if (comptime !std.process.can_spawn) return 1;
    var child: std.process.Child = .init(argv, alloc);

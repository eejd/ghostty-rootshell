...
pub inline fn home(environ_map: *const std.process.Environ.Map, buf: []u8) !?[]const u8 {
    return switch (builtin.os.tag) {
        ...
        // iOS, Mac Catalyst and visionOS don't have a user-writable home
        // directory. (Catalyst reached this arm as `.ios` before Zig 0.16 gave
        // it its own OS tag; keep the same behavior.)
        .ios, .maccatalyst, .visionos => null,
...
pub fn expandHome(environ_map: *const std.process.Environ.Map, path: []const u8, buf: []u8) ExpandError![]const u8 {
    return switch (builtin.os.tag) {
        ...
        // iOS, Mac Catalyst and visionOS don't have a user-writable home
        // directory. (Catalyst reached this arm as `.ios` before Zig 0.16.)
        .ios, .maccatalyst, .visionos => return path,

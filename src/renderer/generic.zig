...
const DisplayLink = switch (builtin.os.tag) {
    ...
    .ios, .maccatalyst, .visionos => *IOSDisplayLink,
    else => void,
};
...
pub fn Renderer(comptime GraphicsAPI: type) type {
    return struct {
        ...
        last_damage: ?std.Io.Timestamp = null,
        ...
        fn reconcileLinkIdleLocked(
            ...
            if (damaged) {
                ...
                if (@divFloor(last.durationTo(now).nanoseconds, std.time.ns_per_ms) <= link_restart_burst_ms)
                    display_link.start() catch {};
                return;
            }
            ...
            if (@divFloor(last.durationTo(now).nanoseconds, std.time.ns_per_ms) > link_idle_pause_ms)
                display_link.stop() catch {};
        }

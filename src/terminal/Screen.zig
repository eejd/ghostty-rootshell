...
fn selectIPv6Address(_: *Screen, pin: Pin) ?Selection {
    ...
    const start_cp = start_cell.codepoint();
...
test "Screen: selectWordOrIPv6 full address" {
    ...
    var s = try init(std.testing.io, alloc, .{ .cols = 50, .rows = 10, .max_scrollback_bytes = 0 });
    defer s.deinit();
...
test "Screen: selectWordOrIPv6 compressed" {
    ...
    var s = try init(std.testing.io, alloc, .{ .cols = 30, .rows = 10, .max_scrollback_bytes = 0 });
    defer s.deinit();
...
test "Screen: selectWordOrIPv6 loopback" {
    ...
    var s = try init(std.testing.io, alloc, .{ .cols = 20, .rows = 10, .max_scrollback_bytes = 0 });
    defer s.deinit();
...
test "Screen: selectWordOrIPv6 IPv4-mapped" {
    ...
    var s = try init(std.testing.io, alloc, .{ .cols = 40, .rows = 10, .max_scrollback_bytes = 0 });
    defer s.deinit();

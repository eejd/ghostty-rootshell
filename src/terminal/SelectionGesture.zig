...
pub const Drag = struct {
    ...
    pub const Geometry = struct {
        ...
        /// rootshell(iOS): optional override for the y position (in surface
        /// pixels) at or above which a downward autoscroll triggers. When null,
        /// the standard `screen_height - autoscroll_buffer` edge is used. iOS /
        /// Mac Catalyst set this because the OS caps the reported pointer y a
        /// logical point short of the drawable bottom, making an edge-exact
        /// bottom trigger unreachable; the caller folds a small slop into the
        /// value and derives it from the true grid bottom rather than
        /// `screen_height`, which overshoots due to padding/row rounding.
        bottom_autoscroll_threshold: ?f64 = null,
    };
};
...
pub fn drag(
    ...
    const bottom_threshold = d.geometry.bottom_autoscroll_threshold orelse
        (max_y - autoscroll_buffer);
    self.left_drag_autoscroll = if (d.ypos <= autoscroll_buffer)
        .up
    else if (d.ypos > bottom_threshold)
        .down
    else
        .none;
...
pub fn deepPress(
    ...
    const click_pin = self.validatedLeftClickPin(&t.screens) orelse return null;
    // rootshell: IPv6-aware word selection (double/force-press selects a
    // full IPv6 address as one unit), falling back to standard word selection.
    const sel = t.screens.active.selectWordOrIPv6(
        click_pin.*,
...
fn pressSelection(
    ...
    return switch (self.left_click_behavior) {
        .cell => null,
        .word => screen.selectWordOrIPv6(p.pin, p.word_boundary_codepoints),

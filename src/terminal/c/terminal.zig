...
fn wrap(
    ...
    handler.effects = .{
        ...
        // libghostty-vt has no per-pane surface to forward this to; only the
        // tmux viewer routes it. ROOTSHELL-TMUX (id=streamterm-pane-osc)
        .pwd_report = null,
        // No tmux viewer here to replay an unwrapped passthrough through, so
        // the inner sequence is dropped. ROOTSHELL-TMUX

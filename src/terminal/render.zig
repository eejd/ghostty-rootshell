...
pub const RenderState = struct {
    ...
    pub fn update(
        ...
        try self.beginUpdateExtraRows(alloc, t, scrollbar, 0);
        self.endUpdate();
    }
    ...
    pub fn updateExtraRows(
        ...
        try self.beginUpdateExtraRows(alloc, t, scrollbar, extra_rows);
        self.endUpdate();
    }
    ...
    pub fn beginUpdate(
        ...
        const scrollbar = t.screens.active.pages.scrollbar();
        return self.beginUpdateExtraRows(alloc, t, scrollbar, 0);
    }

    /// Begin an update of the render state including render-only extra rows
    /// after the viewport when available. Every begin must be completed with an
    /// `endUpdate` call before the render state is read.
    pub fn beginUpdateExtraRows(
        self: *RenderState,
        alloc: Allocator,
        t: *Terminal,
        scrollbar: PageList.Scrollbar,
    ...
    pub fn rebuildViewportRow(
        ...
        const builder: RowBuilder = .{
            ...
            .pending_styles = &self.pending_styles,
            .applied_styles = row_data.items(.applied_styles),
        };

        const pin = row_data.items(.pin)[y];
        const p: *page.Page = pin.node.page();

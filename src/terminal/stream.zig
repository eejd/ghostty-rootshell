...
test "stream: continuation every-byte cuts preserve future behavior" {
    // ROOTSHELL-TMUX (id=parsetable-dcs-utf8-passthrough): in a tmux build the
    // parse table keeps ESC / CAN / SUB / C1 inside dcs_passthrough so the
    // gateway's control-mode DCS isn't cut short by raw bytes in its payload.
    // Upstream's continuation scanner models the stock transitions, so a DCS
    // corpus cut mid-sequence restores different dcs_active state here. The
    // divergence is deliberate and confined to DCS; a non-tmux build runs this
    // test unchanged.

...
test "boundary scanner preserves unfinished builder actions" {
    // ROOTSHELL-TMUX (id=parsetable-dcs-utf8-passthrough): the DCS half of this
    // test asserts stock "anywhere" transitions, but a tmux build deliberately
    // keeps ESC inside dcs_passthrough (so the gateway's control-mode DCS
    // survives raw ESC in its payload), which makes the trailing ESC here
    // uncommitted rather than committed. The APC half is unaffected.

...
pub const Handler = struct {
    ...
    pub const Effects = struct {
        ...
        /// Still fork-only; upstream has no equivalent effect.
    ...
    inline fn vtFallible(
        ...
        switch (action) {
            ...
            // ROOTSHELL-TMUX (id=streamterm-dcs-st): upstream's dcs_handler owns
            // real DCS commands, but the fork must ALSO watch for the string
            // terminator itself, because its parse table won't leave
            // dcs_passthrough on ESC / C1 / high bytes (see `dcs_pending_esc`).
            // ROOTSHELL-TMUX (id=streamterm-tmux-passthrough): and a confirmed
            // `ESC P tmux; ...` envelope is un-doubled and replayed rather than
    ...
    /// ROOTSHELL-TMUX (id=streamterm-dcs-st, id=streamterm-tmux-passthrough):
    /// the fork's half of DCS put, run before upstream's `dcsPut`.
    ///
    /// Returns true when the byte belongs to a confirmed `ESC P tmux; ...`
    /// envelope and has been consumed for unwrapping — those bytes are the
    /// wrapped inner sequence, not a DCS command payload, so upstream's DCS
...
test "OSC 52 clipboard write effect forwards SET, ignores read" {
    // ROOTSHELL-TMUX (id=streamterm-clipboard): tmux relays the app's raw OSC 52
    // bytes in %output, so the pane stream sees them here. A control client
    // (-CC) has no tty, so tmux never forwards the clipboard itself (its
    // `tty_set_selection` path is gated off for control clients); the write is
    // routed to the app through upstream's `clipboard_write` effect, which the
    // viewer installs in `initLayout`. The clipboard READ (`data == "?"`) is a
    // deliberate gap — it needs an async app round-trip and a reply routed back
    // into the pane — so it must never reach the effect.
    var t: Terminal = try .init(testing.io, testing.allocator, .{ .cols = 10, .rows = 10 });
    defer t.deinit(testing.allocator);
    ...
    const S = struct {
        ...
        var len: usize = 0;
        var calls: usize = 0;
        fn cb(_: *Handler, write: clipboard.Write) clipboard.WriteResult {
            calls += 1;
    ...
    // SET: upstream decodes the base64 payload. "aGVsbG8=" = "hello".
    s.nextSlice("\x1b]52;c;aGVsbG8=\x07");
    try testing.expectEqual(@as(usize, 1), S.calls);
    try testing.expectEqual(clipboard.Location.standard, S.location);
    try testing.expectEqualStrings("hello", S.buf[0..S.len]);

    // The selection kind still round-trips to the right destination.
    s.nextSlice("\x1b]52;p;aGVsbG8=\x07");
    try testing.expectEqual(@as(usize, 2), S.calls);
    try testing.expectEqual(clipboard.Location.primary, S.location);
...
test "OSC 9;4 progress / OSC 7 pwd / OSC 9 notification forward to effects" {
    ...
    const S = struct {
        ...
        fn onNotif(_: *Handler, _: Action.ShowDesktopNotification) void {
            notif_calls += 1;
        }
    };
    S.progress_calls = 0;

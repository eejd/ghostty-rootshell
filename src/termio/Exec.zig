...
const Subprocess = struct {
    ...
    fn killPid(pid: c.pid_t) !void {
        ...
        while (true) {
            ...
            // kill them again.
            //
            // NOTE: this must keep using the raw `posix.system.waitpid` rather
            // than the `std.posix.waitpid` wrapper, which panics on unexpected
            // errors (EACCES can happen with pid=1 on Mac Catalyst).
